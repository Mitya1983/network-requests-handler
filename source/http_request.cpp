...
void mt::network::GetRequest::_prepareRequest() {
    if (not m_request_composed) {
        mt::utility::copy(m_request_name.begin(), m_request_name.end(), m_request_data);
        if (m_url.path().empty() || m_url.path().at(0) != '/') {
            m_request_data.push_back(static_cast< std::byte >('/'));
        }
        mt::utility::copy(m_url.path().begin(), m_url.path().end(), m_request_data);
        if (not m_params.empty()) {
            ...
            for (const auto& param: m_params) {
                ...
                mt::utility::copy(param.name.begin(), param.name.end(), m_request_data);
                if (!param.value.empty()) {
                    m_request_data.push_back(static_cast< std::byte >('='));
                    auto encodedValue = utility::encodeUrl(param.value);
                    mt::utility::copy(encodedValue.begin(), encodedValue.end(), m_request_data);
                }
                ++param_count;
            }
        }
        if (const auto& l_query = m_url.query(); not l_query.empty() ) {
            ...
            mt::utility::copy(l_query.begin(), l_query.end(), m_request_data);
        }
        std::string to_insert = " HTTP/1.1\r\n";
        mt::utility::copy(to_insert.begin(), to_insert.end(), m_request_data);
        if (!m_headers.empty()) {
            for (const auto& header: m_headers) {
                mt::utility::copy(header.name.begin(), header.name.end(), m_request_data);
                m_request_data.push_back(static_cast< std::byte >(':'));
    ...
    if (not m_request_composed) {
        ...
        if (!m_headers.empty()) {
            for (const auto& header: m_headers) {
                mt::utility::copy(header.name.begin(), header.name.end(), m_request_data);
                m_request_data.push_back(static_cast< std::byte >(':'));
                mt::utility::copy(header.value.begin(), header.value.end(), m_request_data);
                m_request_data.push_back(static_cast< std::byte >('\r'));
                m_request_data.push_back(static_cast< std::byte >('\n'));
            }
        }
        ...
        if (not m_body.empty()) {
            mt::utility::copy(m_body.begin(), m_body.end(), m_request_data);
        }
        m_request_data.shrink_to_fit();
        m_request_composed = true;
    }
